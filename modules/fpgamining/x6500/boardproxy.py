...
class X6500BoardProxy(Process):
  ...
  def run(self):
    ...
    try:
      ...
      self.fpgas = []
      jtag = JTAG(self, "", self.device, None, 0)
      jtag.tap = TAP(jtag.jtagClock)
      with self.device.mutex:
        jtag.detect()
      self.log(": Found %i device%s\n" % (jtag.deviceCount, 's' if jtag.deviceCount != 1 else ''), 500)
      for i in range(0, jtag.deviceCount):
        self.fpgas.append(FPGA(self,"FPGA"+str(i), self.device, jtag, 0))
      ...
      for id, fpga in enumerate(self.fpgas):
        ...
        fpga.jtag.part(id)
        fpga.detect()
        self.log("FPGA %d: %s - Firmware: rev %d, build %d\n" % (id, JTAG.decodeIdcode(fpga.jtag.idcodes[fpga.id]), fpga.firmware_rev, fpga.firmware_build), 500)
      ...
      send_args = []
      for fpga in self.fpgas:
        send_args.append(fpga.firmware_rev)

      self.send("started_up", send_args)
